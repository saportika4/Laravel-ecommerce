...
class ProductController extends Controller
{
            ...
            if($request->colors){
                foreach($request->colors as $key => $color){
                    ...
                    if($request->colorQuantity[$key] <= 0){
                        return redirect('/admin/products')->with('message','Product Color Quantity Required');
                    }
    ...
    public function destroyImage(int $product_img_id)
    {
        $productImage = ProductImage::findOrFail($product_img_id);
